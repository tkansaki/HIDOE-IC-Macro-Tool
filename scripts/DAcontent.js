        ...
        try {
            ...
            if (innerDoc4) {
                ...
                if (tempNew.dataset.state == "0") {
                    tempNew.dispatchEvent(new KeyboardEvent('keydown', {
                        'key': 'a'
                    }));
                }
            ...
            if (innerDoc4) {
                ...
                if (deleteButton) {
                    ...
                    if (tempDelete.dataset.state == "0") {
                        tempDelete.dispatchEvent(new KeyboardEvent('keydown', {
                            'key': 'a'
                        }));
                    }
        ...
        try {
            ...
            if (innerDoc4) {
                ...
                if (tempSave.dataset.state == "0") {
                    tempSave.dispatchEvent(new KeyboardEvent('keydown', {
                        'key': 'a'
                    }));
                }
...
async function addDA(schools, values) {
    if (values && schools) {
        ...
        if (notAdded.length > 0) {
            ...
            setTimeout(function () {
                alert(`Failed to add District Assignments for schools:\n\n${msg}`)
            }, 1000);
        } else {
            setTimeout(function () {
                alert("Successfully completed adding all District Assignments")
            }, 1000);
        }
    }
}
...
async function editDA(schools, matchValues, setValues) {
    if (schools && matchValues && setValues) {
        ...
        if (failed.size > 0) {
            ...
            setTimeout(function () {
                alert(`Failed to edit District Assignments for schools:\n\n${msg}`)
            }, 1000);
        } else {
            setTimeout(function () {
...
async function deleteDA(schools, values) {
    if (schools && values) {
        ...
        if (failed.size > 0) {
            ...
            setTimeout(function () {
                alert(`Failed to delete District Assignments for schools:\n\n${msg}`)
            }, 1000);
        } else {
            setTimeout(function () {
