...
chrome.runtime.onMessage.addListener(
  async function(request, sender, sendResponse) {
    if(request.valueForm && request.booleanForm && request.schools) {
      let notAdded = [];
      for (let i = 0; i < request.schools.length; i++){
        let cancel = false;
        let repeat = false;
        await new Promise((resolve) => pressNew(resolve));
        await new Promise((resolve) => waitForDetails(resolve));
        await new Promise((resolve) => fillForm(request.schools[i], request.valueForm, request.booleanForm, resolve)).then(function(save) {
          if (!save) {
            cancel = true;
          }
        });
        if (!cancel) {
          await new Promise((resolve) => pressSave(request.schools[i], resolve));
        }
        ...
        if (repeat) {
          ...
          await new Promise((resolve) => checkIfSaved(resolve)).then(function(saved) {
            if (!saved) {
              notAdded.push(request.schools[i])
            }
          });
        }
      }
